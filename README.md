...
2. Загружаете зависимости (папку vendor). В папке бота делаете
```
composer install
```
Если нет возможности сделать на хостинге, делаете у себя и папку vendor заливаете на хостинг.
3. В файле bot.php в строке 5 прописать свой токен, который получен у BotFather
4. Добавить для крона задачу вида:
```
